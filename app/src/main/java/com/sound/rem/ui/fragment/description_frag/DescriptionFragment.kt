            ...
            .doOnNext {property ->
                if (property != null){
                    ...
                    if (property.lat != null && property.lng != null)
                        mapFrag.dataChangeSimpleMap(LatLng(property.lat!!,property.lng!!))
    ...
    private fun updateUi(){
        ...
        dbViewModel.isDollar
            ...
            .doOnNext{
                ...
                val tmp:Long
                if (property.isDollar && it == false){
                    tmp = Utils.convertDollarToEuro(property.price, dbViewModel.usdEur)
                }else if (!property.isDollar && it == true) {
                    tmp = Utils.convertEuroToDollar(property.price, dbViewModel.usdEur)
                }else{
                    tmp = property.price
                }
                tvPrice_desc.text = Utils.priceToString(tmp, it)
            }.subscribe().addTo(compositeDisposable)
